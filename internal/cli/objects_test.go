...
func TestObjectVerbRequests(t *testing.T) {
	cases := []struct {
		...
		{[]string{"companies", "restore", testID}, "PATCH", "/rest/restore/companies", map[string]string{"filter": "id[eq]:" + testID}, ""},
		{[]string{"companies", "restore", testID, "--depth", "1"}, "PATCH", "/rest/restore/companies",
			map[string]string{"filter": "id[eq]:" + testID, "depth": "1"}, ""},
...
func TestGatesRefuseBeforeNetwork(t *testing.T) {
	...
	cases := []struct {
		...
		{"filter in restore id", []string{"companies", "restore", testID + ",name[neq]:x"}, "not a record ID"},
